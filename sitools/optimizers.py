...
from sitools.utils import extract_weight_changes, compute_updates
...
class KOOptimizer(Optimizer):
    ...
    def __init__(self, opt, step_updates=[], task_updates=[], init_updates=[], task_metrics = {}, 
        ...
        use the name 'grads' or 'deltas' as those are reserved to contain the gradients
        of the loss, and the weight updates at each step.
        ...
        # task_metrics
        self.names = set().union(init_updates.keys(), step_updates.keys(), task_updates.keys(), task_metrics.keys())
    ...
    def get_updates(self, params,loss,model=None):
        ...
        if self.compute_average_loss:
            with tf.variable_scope("ema") as scope:
                ...
                self.maintain_averages_op = ema.apply([self.loss])
                self.ema_loss = ema.average(self.loss)
